...
在本文写作中，我确实高度依赖与 AI 的交互，但有一个非常重要的原则：**不能把主线思考外包给 AI**。学习路径必须由自己主导，AI 仅作为辅助工具，用于加速局部任务，例如资料查询、示例生成或理解验证，而不能替代整体思考过程。
...
整篇文成文整体用了1.5h，算是写得很快的。但它本身只是一个随手记，不是知识框架里的一部分，就当个阅后即弃的东西吧。